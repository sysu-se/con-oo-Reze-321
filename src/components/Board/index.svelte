<script>
	...
	import { gamePaused } from '@sudoku/stores/game'; // 可以先保留（不影响）
			...
			{#each $game.grid as row, y}
				{#each row as value, x}
					<Cell
						...
						sameNumber={$settings.highlightSame && value && !isSelected($cursor, x, y) && getValueAtCursor($game.grid, $cursor) === value}
